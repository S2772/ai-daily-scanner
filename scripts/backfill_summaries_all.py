...
def main() -> int:
    ...
    # Ensure only one backfill runs at a time.
    ...
    cur.execute(
        ...
        FROM hotspots
        WHERE (ai_summary IS NULL OR trim(ai_summary)='' OR ai_summary LIKE '[PENDING]%')
    ...
    print(f"all candidates={len(to_fix)} (limit={args.limit})")
